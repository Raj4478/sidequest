...
interface DashboardData {
  ...
  resolution?: any;
  stats?: { days_remaining: number; milestones_total: number; milestones_completed: number; milestones_overdue: number; };
  upcoming_milestone?: any;
  last_reflection?: any;
}
...
function ProgressRing({ percent, size = 120 }: { percent: number; size?: number }) {
  ...
  return (
    <svg width={size} height={size} style={{ transform: 'rotate(-90deg)' }}>
      ...
      <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke="var(--accent)"
...
export default function DashboardPage() {
  ...
  if (!data?.hasResolution) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh', padding: 24, textAlign: 'center' }}>
      ...
      <Link href="/dashboard/resolution/new" style={{
  ...
  return (
    <div style={{ padding: '40px 32px', maxWidth: 900, margin: '0 auto' }}>
      ...
      <div className="animate-fade-up animate-delay-1" style={{
        ...
        <Link href={`/dashboard/resolution/${resolution.id}`} style={{
        ...
        <div className="animate-fade-up animate-delay-2" style={{
          ...
          {upcoming_milestone ? (
            <>
              ...
              <Link href={`/dashboard/milestones/${upcoming_milestone.id}`} style={{
      ...
      <div className="animate-fade-up animate-delay-4" style={{
        ...
        }}>🗺️ View journey map</Link>
        <Link href={`/dashboard/resolution/${resolution.id}`} style={{
          padding: '10px 18px', borderRadius: 10, border: '1px solid var(--border)',
          color: 'var(--text-secondary)', textDecoration: 'none', fontSize: 13, fontWeight: 500
        }}>➕ Add milestone</Link>
        <Link href="/dashboard/reflections" style={{
          padding: '10px 18px', borderRadius: 10, border: '1px solid var(--border)',
          color: 'var(--text-secondary)', textDecoration: 'none', fontSize: 13, fontWeight: 500
        }}>📝 All reflections</Link>
      </div>
    </div>
  );
}
